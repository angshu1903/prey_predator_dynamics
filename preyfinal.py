...
"""
Created on Wed Jul 13 22:32:20 2022
...
N=250 #number of prey initially
...
step_list=[0,200,1250,5000,9999]

for i in step_list:
    pos_arr_final=(sol.y)[:,i].reshape((-1,2))
    plt.figure()
    plt.rcParams['figure.figsize'] = (10,5)
    plt.rcParams['font.size'] = 16
    plt.rcParams['figure.facecolor'] = 'white'
    plt.grid(ls=':')
    plt.scatter(pos_arr_final[:-1,0],pos_arr_final[:-1,1],color='white',edgecolor='black',label='prey')
    plt.scatter(pos_arr_final[-1,0],pos_arr_final[-1,1],color='red',edgecolor='black',label='predator')
    #print(t_points[i])
    plt.xlabel('x')
    plt.ylabel('y')
    plt.legend(loc=[1.05,0.4], numpoints=1, fancybox=True, frameon=True)
    plt.title(f'time = {t_points[i]:.2f} seconds')
